...
def download_part(filename, length_of_chunk, i):
    ...
    with open(filename, "wb") as file_obj:
        progress = tqdm.tqdm(range((end - start)*BUFFER_SIZE), f"Tiến trình part {i}: ", unit="B", unit_scale=True, unit_divisor=1024)
        while total_received < end - start:
            data, server = child_client.recvfrom(BUFFER_SIZE + 8)
            offset = data[0:4]
            offset = struct.unpack('!I', offset)[0]
            check_sum = data[4:8]
            check_sum = struct.unpack('!I', check_sum)[0]
            content = data[8:len(data)]
            if check_sum == count_sum(content):
